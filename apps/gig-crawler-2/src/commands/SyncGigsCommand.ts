...
export class SyncGigsCommand {
  ...
  async execute(options: { clearExisting?: boolean } = {}): Promise<SyncStats> {
    ...
    try {
      ...
      const query = `rock metal indie folk post-rock dark goth alternative συναυλίες Αθήνα ${dateRange} -pop -dance -techno -rave`;

      const searchResults = await this.search.search(query, 30); // Increased from 20
